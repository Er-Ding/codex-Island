...
flowchart LR
    ...
    E --> B
    B --> F[WPF 灵动岛]
    G[悬停 / 固定 / 拖动] --> H[IslandInteraction]
    H --> F
...
macOS 的 Swift 源码和构建脚本保留原样；Windows 放在独立 `windows/` 目录。业务规则采用等价的 C# 实现，通过测试保持一致，当前不建立 Swift/C# 跨语言运行时共享。未来若统一两个平台，应先共享协议样本与行为测试，再决定是否重构桌面框架。
...
6. **持久化**：设置按用户保存在 LocalAppData，采用临时文件替换。保存失败时保留草稿并显示提示；损坏设置采用默认位置；外接屏暂时不存在时不覆盖原屏幕 ID。
...
Windows 0.3.1 包含 40 项核心检查和 23 项 WPF 窗口检查。运行方法、包位置、已验证环境和剩余实机验收项见 [Windows README](../windows/README.md)。当前实现额度查询和主要交互，不添加账号管理、声音提醒、开机启动或额度重置。
