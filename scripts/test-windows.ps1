...
& $dotnet build (Join-Path $projectRoot 'windows\CodexIsland.slnx') -c Release --nologo
if ($LASTEXITCODE -ne 0) { throw 'Windows build failed.' }
& $dotnet (Join-Path $projectRoot 'windows\CodexIsland.Checks\bin\Release\net10.0\CodexIsland.Checks.dll')
if ($LASTEXITCODE -ne 0) { throw 'Core or transport checks failed.' }
$app = Join-Path $projectRoot 'windows\CodexIsland.Windows\bin\Release\net10.0-windows\CodexIsland.dll'
if ($IncludeUI) {
    $outputDirectory = Join-Path $projectRoot ('.build\windows-ui-' + [Guid]::NewGuid().ToString('N'))
